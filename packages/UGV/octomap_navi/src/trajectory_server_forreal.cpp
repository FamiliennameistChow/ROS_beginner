...
private:
    ...
    std::mutex m_buf_;
    double sum;
    double vehicel_angular_vz_;
    std::queue<geometry_msgs::Vector3Stamped::ConstPtr> imu_acc_buffer_;
...
private:
    ...
    void imuAccCallback(const geometry_msgs::Vector3Stamped::ConstPtr &msg);
...
sum(0.0)
{
	...
	n_.param<string>("/imu_topic", imu_topic_, "/integrated_to_init"); //这个是全局参数,在config文件中修改
    n_.param<string>("/imu_acc_topic", imu_acc_topic_, "/filter/free_acceleration");
    ...
    private_nh.param<double>("Kp_angular", Kp_angular_, 0.3); //　角速度环pid参数
    private_nh.param<double>("Ki_angular", Ki_angular_, 0.0); //　角速度环pid参数
    private_nh.param<double>("Kd_angular", Kd_angular_, 0.0); //　角速度环pid参数


    odom_sub_ = n_.subscribe<nav_msgs::Odometry>(odom_topic_, 1, &TrajectoryServer::odomCallback, this);
	imu_sub_ = n_.subscribe<sensor_msgs::Imu>(imu_topic_, 1, &TrajectoryServer::imuCallback, this);
    imu_acc_sub_ = n_.subscribe<geometry_msgs::Vector3Stamped>("/filter/free_acceleration", 1, &TrajectoryServer::imuAccCallback, this);
...
void TrajectoryServer::imuCallback(const sensor_msgs::Imu::ConstPtr &msg){
	vehicel_angular_vz_ = msg->angular_velocity.z;
}

void TrajectoryServer::imuAccCallback(const geometry_msgs::Vector3Stamped::ConstPtr &msg){
    m_buf_.lock();
    imu_acc_buffer_.push(msg);
    m_buf_.unlock();	
}
...
void TrajectoryServer::TrajFollow(){
    ...
    int index  = 0;
	sum = 0.0;
	if(!imu_acc_buffer_.empty()){
		imu_acc_buffer_.pop();
	}
    double dis, dis_end_to_me, dis_start_to_end, dis_start_to_me;
    int index_last = 1;
	int rotate = -1;
    ...
    //角速度pid控制器变量
    double cmd_angular_vz =0.0;
    double angular_error = 0.0;
    double angular_error_pre = 0.0;
    double angular_error_intergral = 0.0;
    ...
    while (ros::ok())
    {
        ...
        m_buf_.lock();
        double t1, t2, a_x;
        while (!imu_acc_buffer_.empty())
        {
            t1 = imu_acc_buffer_.front()->header.stamp.toSec();
            a_x = imu_acc_buffer_.front()->vector.x;
            imu_acc_buffer_.pop();
            if (imu_acc_buffer_.empty())
            {
                break;
            }
            t2 = imu_acc_buffer_.front()->header.stamp.toSec();
            sum += a_x * (t2-t1);
        };
        ...
        // 角速度pid控制
        angular_error = omega - vehicel_angular_vz_;
        angular_error_intergral += angular_error;
        cmd_angular_vz = Kp_angular_ * angular_error + Ki_angular_* angular_error_intergral + Kd_angular_ * (angular_error - angular_error_pre);
        angular_error_pre = angular_error;

        geometry_msgs::Twist cmd;
        if (abs(theta) > PI/10 && traj_track_state_.data > 0.9)
        {
            cmd.linear.x = 0.0;
            cmd.angular.z = omega/abs(omega) * 0.1; // omega;
			rotate = 5;
        }else
        {
			while(rotate>0){
        ...
        if (index != index_last)
        {
            ...
            cout << " actual_vehicel_angular_vz " << vehicel_angular_vz_ << endl;
            cout << " omega " << omega << endl;
            cout << "cmd_angular_vz" << cmd_angular_vz << endl;
        }
