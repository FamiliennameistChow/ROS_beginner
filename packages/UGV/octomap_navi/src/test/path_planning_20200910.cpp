...
planner::planner(ros::NodeHandle &nh, ros::NodeHandle &private_nh) : n_(nh)
{
	...
	private_nh.param<int>   ("bazier/poly_order",  traj_order_, 8);
	private_nh.param<double>("bazier/min_order",   minimize_order_, 2.5);
	...
	if(bernstein_.setParam(3, 12, minimize_order_) == -1)
        ROS_ERROR(" The trajectory order is set beyond the library's scope, please re-set "); 

	MQM_ = bernstein_.getMQM()[traj_order_];
...
void planner::plan(void){
	...
	Eigen::Vector3d p_start(start_point_.x, start_point_.y, start_point_.z);
    pos.row(0) = p_start;
	Eigen::Vector3d p_end(goal_point_.x, goal_point_.y, goal_point_.z);
    pos.row(1) = p_end;    
    ...
    cout << "vel :" << vel << endl;
    cout << "acc :" << acc << endl;
    cout << "MAX_Vel :" << MAX_Vel_ << endl;
    cout << "MAX_Acc :" << MAX_Acc_ << endl;
    cout << "traj_order: " << traj_order_ << endl;
    cout << "minimize_order :" << minimize_order_ << endl;
    cout << "cube_margin :" << cube_margin_ << endl;
    cout << "is_limit_vel :" << is_limit_vel_ << endl;
    cout << "is_limit_acc :" << is_limit_acc_ << endl;
    cout << "obj :" << obj << endl;
    cout << "bezier_coeff :" << bezier_coeff_ << endl;
	...
	if (trajectoryGenerator_.BezierPloyCoeffGeneration
