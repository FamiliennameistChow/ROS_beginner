...
typedef unordered_map<octomap::point3d, octomap::point3d> Point_pair_set; //储存(当前节，父节点)对
...
octomap::point3d planner::Near(Point_pair_set &V ,octomap::point3d p){
	float min = Norm(V.begin()->first, p);
    float temp;
...
void planner::backTree(octomap::point3d p, octomap::point3d p_init, vector<octomap::point3d> &rrt_tree, float &cost){
	...
	for (size_t i = 0; i < rrt_tree_set.size(); i++)
	{
		auto iter = rrt_tree_set.find(point_back);
		cost += Norm(point_back, iter->second);
		rrt_tree.push_back(point_back);
		point_back = iter->second;
		if (point_equal(point_back, p_init))
		{
			rrt_tree.push_back(point_back);
			return;
...
void planner::rewirteTree(Point_pair_set &V, octomap::point3d new_point, octomap::point3d nearest_point, float search_r){
	...
	backTree(nearest_point, V.begin()->first, nearest_point_cost);
	...
	for (auto& vp : V)
	{
		...
		if( dis<= search_r){
			...
			backTree(vp.first, V.begin()->first, cost);
	...
	for (multimap<float, octomap::point3d>::iterator it = cost_point_pair.begin(); it != cost_point_pair.end(); it++)
	{
		...
		{
			V.insert(make_pair(p_new, nearest_point));
			return;
		}
...
void planner::plan(void){
	...
	line.points.clear();
	rrt_tree_set.insert(make_pair(base_start, base_start));
	...
	while (ros::ok())
	{
		...
		if (checking == 1)
		{
			...
			{	
				...
				rrt_tree_set.insert(make_pair(base_goal, p_new));
				...
				p_back = base_goal;
				for(int i = 0; i < rrt_tree_set.size(); i++){
					auto iter = rrt_tree_set.find(p_back);
					rrt_path.push_back(p_back);
					p_back = iter->second;
					// vis
					p_pub= iter->first;
