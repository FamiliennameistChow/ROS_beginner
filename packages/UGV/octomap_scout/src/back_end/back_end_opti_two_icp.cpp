...
private:
    ...
    pcl::PointCloud<PointType>::Ptr RSLoopKeyFrameCloud;  //回环关键帧对应点云
...
void BackEndOpti::init_params(ros::NodeHandle &nh_){
    ...
    RSLoopKeyFrameCloud.reset(new pcl::PointCloud<PointType>);
...
bool BackEndOpti::timeSyncData(){
    while (!scan_pc_buf_.empty() && !laser_odom_buf_.empty()) 
    {
        ...
        cout << "time sync: " << time_sync.toc() << " ms" << endl;
        mBuf.unlock();
        return true;
    }
...
bool BackEndOpti::insertKeyFrame(){
    ...
    if (save_this_key_frame)
    {
        ...
        cout << "time insert keyFrame: " << time_insert_key_frame.toc() << " ms" <<  endl;
        return true;
    ...
    */ 
    ...
    }else // 找到回环,保存关键帧信息
    {
        ...
        *RSlatestSurfKeyFrameCloud = *RShahaCloud;
        ...
        //回环关键帧点云　test
        *RSLoopKeyFrameCloud += *pcTransToInit(pc_key_frames_[RSclosestHistoryFrameID], cloudKeyPoses6D->points[RSclosestHistoryFrameID]);

        // pcl::PointCloud<PointType>::Ptr RSnearHistorySurfKeyFrameCloudDS_loopTF(new pcl::PointCloud<PointType>());
        // pcl::transformPointCloud(*RSnearHistorySurfKeyFrameCloudDS, *RSnearHistorySurfKeyFrameCloudDS_loopTF, 
        //                         (pclPointToAffine3f(cloudKeyPoses6D->points[RSclosestHistoryFrameID]).matrix()).inverse());
        ...
        // pcl::io::savePCDFileASCII(pcd_file_path + to_string(RSclosestHistoryFrameID) + ".pcd", *RSLoopKeyFrameCloud);
        //<<<<<<<<<
...
void BackEndOpti::performLoopClosure(){
    ...
    Eigen::Affine3f t_latest_wrong_to_right, t_loop_wrong_to_right;
    ...
    if( RSclosestHistoryFrameID != -1 ) {
        pcl::IterativeClosestPoint<PointType, PointType> icp; // latest 
        ...
        pcl::IterativeClosestPoint<PointType, PointType> icp2; // loop
        ...
        icp2.setInputSource(RSLoopKeyFrameCloud);
        icp2.setInputTarget(RSnearHistorySurfKeyFrameCloudDS);
        // pcl::PointCloud<PointType>::Ptr unused_result(new pcl::PointCloud<PointType>());
        icp2.align(*unused_result);

        std::cout << "[RS] ICP1 fit score: " << icp.getFitnessScore() << std::endl;
        std::cout << "[RS] ICP2 fit score: " << icp2.getFitnessScore() << std::endl;
        if ( icp.hasConverged() == false || icp.getFitnessScore() > historyKeyframeFitnessScore || 
             icp2.hasConverged() == false || icp2.getFitnessScore() > historyKeyframeFitnessScore) {
        ...
        if( isValidRSloopFactor == true ) {
            t_latest_wrong_to_right = icp.getFinalTransformation(); // get transformation in camera frame (because points are in camera frame)??

            //>>>>>>test
            pcl::getTranslationAndEulerAngles (t_latest_wrong_to_right, x, y, z, roll, pitch, yaw);
            ...
            Eigen::Affine3f t_latset_wrong_to_init = pclPointToAffine3f(cloudKeyPoses6D->points[latestFrameIDLoopCloure]);

            //>>>>>>test
            pcl::getTranslationAndEulerAngles (t_latset_wrong_to_init, x, y, z, roll, pitch, yaw);
            cout << "t_currentOld_to_init: " <<latestFrameIDLoopCloure<< " id " << x << " " << y << " " << z << " " << roll << " " << pitch << " " << yaw << endl;
            //<<<<<<<

            // transform from world origin to corrected pose
            Eigen::Affine3f t_latset_right_to_init = t_latest_wrong_to_right * t_latset_wrong_to_init; // pre-multiplying -> successive rotation about a fixed frame
            pcl::getTranslationAndEulerAngles (t_latset_right_to_init, x, y, z, roll, pitch, yaw);
            ...
            // 计算回环关键帧 最新位姿
            t_loop_wrong_to_right = icp2.getFinalTransformation();
            Eigen::Affine3f t_loop_wrong_to_init = pclPointToAffine3f(cloudKeyPoses6D->points[RSclosestHistoryFrameID]);
            Eigen::Affine3f t_loop_right_to_init = t_loop_wrong_to_right * t_loop_wrong_to_init;
            pcl::getTranslationAndEulerAngles (t_loop_right_to_init, x, y, z, roll, pitch, yaw);
            gtsam::Pose3 poseTo = Pose3(Rot3::RzRyRx(yaw, pitch, roll), Point3(x, y, z));
            cout << "----1111111--------" << endl;
            //>>>>>>test
            ...
            gtSAMgraph.resize(0);
            aLoopIsClosed = true;
            cout << "time loop : " << time_loop.toc() << " ms " << endl;
            // 可视化回环关系
