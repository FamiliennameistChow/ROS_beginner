...
public:
  ...
  sensor_msgs::NavSatFix globalPosition(); //获取无人机的global_positon坐标
  void pubLocalPos(geometry_msgs::PoseStamped setpoint); // 发布local positon的设定值


  void uploadMission(const std::string& qgc_plan_file); // 从QGC地面站生成的任务列表中上传航点任务
  ...
  // void moveGPS(mavros_msgs::GlobalPositionTarget gps_point); // 将无人机移动到某个GPS坐标下
  void moveGPS(geographic_msgs::GeoPoseStamped gps_point);
...
private:
  ...
  // mavros_msgs::GlobalPositionTarget set_gps_position;
  geographic_msgs::GeoPoseStamped set_gps_position;
  ...
  void getglobalPositionCB(const sensor_msgs::NavSatFix::ConstPtr &msg);
...
AeroDrone::AeroDrone()
{
  ...
  // global_pgs_pub_ = nh_.advertise<mavros_msgs::GlobalPositionTarget>("/mavros/setpoint_raw/global", 10);
  global_pgs_pub_ = nh_.advertise<geographic_msgs::GeoPoseStamped>("/mavros/setpoint_position/global", 10);
  ...
  :uav_num_(uav_num)
{
  ...
  // global_pgs_pub_ = nh_.advertise<mavros_msgs::GlobalPositionTarget>(topic_head_ + "/mavros/setpoint_raw/global", 10);
  global_pgs_pub_ = nh_.advertise<geographic_msgs::GeoPoseStamped>(topic_head_ + "/mavros/setpoint_position/global", 10);
...
void AeroDrone::getglobalPositionCB(const sensor_msgs::NavSatFix::ConstPtr &msg){
  global_position = *msg;
}
...
void AeroDrone::watchFlightModeThread()
{
  ...
  // global position
  ros::Subscriber global_position_sub = nh_.subscribe<sensor_msgs::NavSatFix>(
    topic_head_ + "/mavros/global_position/global", 5, boost::bind(&AeroDrone::getglobalPositionCB, this, _1));
...
void AeroDrone::publishThread()
{
  while(ros::ok())
  {
    ...
    else
    {
      if (pub_gps_flag_)
      {
        global_pgs_pub_.publish(set_gps_position);
        ros::spinOnce();
        rate_.sleep();
      }else
      {
...
sensor_msgs::NavSatFix AeroDrone::globalPosition()
{
...
// void AeroDrone::moveGPS(mavros_msgs::GlobalPositionTarget gps_point){
//   set_gps_position = gps_point;
//   pub_gps_flag_ = true;
// }

void AeroDrone::moveGPS(geographic_msgs::GeoPoseStamped gps_point){
  set_gps_position = gps_point;
  pub_gps_flag_ = true;
}
