...
// 相机内参
float depth_camera_fx, depth_camera_fy, depth_camera_cx, depth_camera_cy; // 深度相机
float rgb_camera_fx, rgb_camera_fy, rgb_camera_cx, rgb_camera_cy; // 深度相机
// float bottom_camera_fx, bottom_camera_fy, bottom_camera_cx, bottom_camera_cy; // 下置相机
...
void findCircleFromDepth(){
    ...
    Mat binaryImage, filterImage, img_show;//临时变量和目标图的定义
    // cout << img_depth << endl;
    // Mat_<float> imgf_sub = img_depth / 1000; //将深度图数值转换为实际深度（float）
    Mat imgf_sub;
    img_depth.convertTo(imgf_sub, CV_32F, 0.001); //将深度图数值转换为实际深度（float）
    // cout <<"imgf: " <<  imgf_sub << endl;

    imshow("img_depth", imgf_sub);
    waitKey(1);
    ...
    inRange(imgf_sub, 0.3, 3, binaryImage);
    GaussianBlur(binaryImage, filterImage, Size(7, 7), 2, 2 ); // 滤波

    imshow("filterImage", filterImage);
    waitKey(1);

    cvtColor(filterImage, img_show, CV_GRAY2BGR); // 显示用

    //【4】进行霍夫圆变换
    vector<Vec3f> circles;
    HoughCircles(filterImage, circles, CV_HOUGH_GRADIENT, 1, 20, 200, 60, 80, 0 );

    //【5】依次在图中绘制出圆
    Point center_in_pixel;
    int radius;
    cout << "[find circle] circle size: " << circles.size() << endl;
    if (circles.size() == 0)
    {
        return;
    }
    ...
    //绘制圆心
    circle(img_show, center_in_pixel, 3, Scalar(255, 0, 0), -1, 8, 0 );
    //绘制圆轮廓
    circle(img_show, center_in_pixel, radius, Scalar(0 , 255, 255), 3);
    ...
    Mat img_depth_roi = imgf_sub(roi);
    ...
    cout << "[find circle] center pose in pixel:  " << center_in_pixel.x << " " << center_in_pixel.y << " " << center_point_in_camrea.z << " radius " << radius<< endl;
    // 像素坐标转相机坐标
    // Xc = (u - u0)*Zc / fx
    // 

    cout << "[find circle] img_depth.cols/2: "  << img_depth.cols/2 << " img_depth.rows/2 " << img_depth.rows/2 << endl; 
    center_point_in_camrea.x = (center_in_pixel.x - img_depth.cols/2) * center_point_in_camrea.z / depth_camera_fx;
    center_point_in_camrea.y = (center_in_pixel.y - img_depth.rows/2) * center_point_in_camrea.z / depth_camera_fy;

    // center_point_in_camrea.x = (center_in_pixel.x - depth_camera_cx) * center_point_in_camrea.z / depth_camera_fx;
    // center_point_in_camrea.y = (center_in_pixel.y - depth_camera_cy) * center_point_in_camrea.z / depth_camera_fy;
    ...
    cout << "[find circle] reslut in body: " << center_point_in_body.x <<" " <<center_point_in_body.y << " " << center_point_in_body.z << endl;
    
    front_camera_reslut_pub.publish(center_point_in_body);

    // // //【6】显示效果图  
    imshow("process", img_show);
    waitKey(1); 
    sensor_msgs::ImagePtr msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", img_show).toImageMsg();
...
void findCircleFromYOLO(){
    ...
    // Xc = (u - u0)*Zc / fx
    // 
    center_point_in_camrea.x = (center_in_pixel.x - img_front.cols/2) * center_point_in_camrea.z / rgb_camera_fx;
    center_point_in_camrea.y = (center_in_pixel.y - img_front.rows/2) * center_point_in_camrea.z / rgb_camera_fy;
...
int main(int argc, char **argv)
{
    ...
    // 相机内参 [全局]
    // depth camera
    nh_.param<float>("depth_camera/fx",    depth_camera_fx,   387.229248046875);
    nh_.param<float>("depth_camera/fy",    depth_camera_fy,   387.229248046875);
    nh_.param<float>("depth_camera/cx",    depth_camera_cx,   321.04638671875);
    nh_.param<float>("depth_camera/cy",    depth_camera_cy,   243.44969177246094);
    // rgb camera
    nh_.param<float>("rgb_camera/fx",      rgb_camera_fx,   387.229248046875);
    nh_.param<float>("rgb_camera/fy",      rgb_camera_fy,   387.229248046875);
    nh_.param<float>("rgb_camera/cx",      rgb_camera_cx,   321.04638671875);
    nh_.param<float>("rgb_camera/cy",      rgb_camera_cy,   243.44969177246094);
    // bottom camera
    // nh_.param<float>("bottom_camera/fx",   bottom_camera_fx,   387.229248046875);
    // nh_.param<float>("bottom_camera/fy",   bottom_camera_fy,   387.229248046875);
    // nh_.param<float>("bottom_camera/cx",   bottom_camera_cx,   321.04638671875);
    // nh_.param<float>("bottom_camera/cy",   bottom_camera_cy,   243.44969177246094);
